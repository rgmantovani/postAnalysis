...
getRuntimeLinePlot = function(runtime.data) {

  temp = runtime.data
  aux = temp[order(temp$usercpu.time.millis, decreasing = FALSE),]
  aux$alg = factor(aux$alg, levels = aux$alg)
  colnames(aux) = c("training", "testing", "total", "alg")
  aux.final = melt(aux, id.vars = 4)   
  colnames(aux.final) = c("alg", "time", "value")

  g = ggplot(data=aux.final, 
    aes(x=alg, y=log(value), colour=time, group=time, linetype=time, shape=time))
